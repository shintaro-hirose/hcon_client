...
function CSVExportButton({userData}) {
  ...
  if( results && (results.length !== 0) ){
    ...
    if(thirdDnfReason !== ""){
      ...
      newResults.push({
          firstTime,secondTime,thirdTime,firstDnfReason, secondDnfReason, thirdDnfReason, bestTime, createdAt
      })
    })
  }
  ...
  return (
    <div>
      ...
      >
        <DialogTitle id="csv_export-dialog-title">{"CSVでエクスポート"}</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <CSVLink data={newResults ? newResults : []} headers={headers} filename={`hcon_${userHandle}_results.csv`}>
